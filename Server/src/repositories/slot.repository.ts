  ...
  implements ISlotRepository
{
  ...
  async create(
    scheduleData: Partial<IDoctorScheduleDocument>,
    session?: ClientSession | undefined,
  ): Promise<IDoctorScheduleDocument> {
  ...
  async removeBlockedDate(
    ...
    if (schedule.blockedDates) {
      const index = schedule.blockedDates.findIndex(
        (blocked) => blocked.date.toISOString().split('T')[0] === dateStr,
      );

      if (index >= 0) {
        schedule.blockedDates.splice(index, 1);
        schedule.markModified('blockedDates');
        return await schedule.save({ session: session || null });
      }
    }
