  ...
  implements IDoctorRepository
{
    ...
    if (filter) {
      ...
      if ((filter as any).hasSlots) {
        ...
        const schedules = (await DoctorScheduleModel.find({
          ...
          .select('doctorId')
          .lean()) as any as { doctorId: Types.ObjectId }[];
  ...
  async getPendingDoctorRequests(): Promise<DoctorRequestItem[]> {
    ...
    return docs
      ...
      .map((doc) => this._mapDoctorRequestItem(doc));
  }
  ...
  async getDoctorRequestDetailsById(
    doctorId: string,
  ): Promise<DoctorRequestDetail | null> {
    const doc = await this.model.findById(doctorId).populate('userId').lean();

    if (!doc) return null;

    const user = doc.userId as unknown as IUserDocument;
    if (!user) {
      return null; // Or throw error, but null is safer for a "getById"
    }
  ...
  private _mapDoctorRequestItem(
    doc: PopulatedDoctorRequest,
  ): DoctorRequestItem {
    const user = doc.userId;
    if (!user) {
      throw new Error(
        `Doctor record ${doc._id} is missing its associated user`,
      );
    }
    return {
      _id: doc._id,
