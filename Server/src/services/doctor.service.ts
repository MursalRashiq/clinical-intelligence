...
export class DoctorService implements IDoctorService {
  ...
  async submitVerification(
    ...
    this._logger.info(
      `Submitting verification for doctor with userId: ${userId}`,
    );

    const doctor = await this._doctorRepository.findByUserId(userId);
    if (!doctor) {
      throw new NotFoundError(
        'Doctor profile not found. Please complete step 1 first.',
      );
    }
    ...
    const updatedDoctor = await this._doctorRepository.updateById(
      doctor._id.toString(),
      updateData,
    );
  ...
  async resubmitVerification(userId: string): Promise<IDoctorDocument> {
    this._logger.info(
      `Resubmitting verification for doctor with userId: ${userId}`,
    );
  ...
  async updateDoctorDocuments(
    ...
    if (files.medicalLicense && files.medicalLicense.length > 0) {
      const file = files.medicalLicense[0];
      if (!file) {
        throw Error('medicalLicense is required');
      }
    ...
    if (files.degreeCertificate && files.degreeCertificate.length > 0) {
      const file = files.degreeCertificate[0];
      if (!file) {
        throw Error('Degree certificate is required');
      }
  ...
  async getDocumentSignedUrl(userId: string, index: number): Promise<string> {
    this._logger.info(
      `Generating signed URL for document ${index} for doctor ${userId}`,
    );
    const doctor = await this._doctorRepository.findByUserId(userId);
    if (
      !doctor ||
      !doctor.verificationDocuments ||
      !doctor.verificationDocuments[index]
    ) {
