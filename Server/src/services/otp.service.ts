...
export class OTPService implements IOtpService {
  ...
  async createAndSendOtp(
    ...
    await this._otpRepository.createOrUpdateOtp(email, {
      otp,
      userData,
      otpExpiresAt,
      expiresAt,
  ...
  async createPasswordResetOtp(
    ...
    const existingOtp = await this._otpRepository.findOneByField(
      'email',
      email,
    );
  ...
  async verifyResetToken(email: string, resetToken: string): Promise<OTPData> {
    const otpRecord = await this._otpRepository.findByEmailAndOtp(
      email,
      resetToken,
    );
