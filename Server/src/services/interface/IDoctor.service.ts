...
export interface IDoctorService {
    ...
    getAllApprovedDoctors(filter?: any): Promise<{ doctors: IDoctorDocument[]; total: number }>;
    resubmitVerification(userId: string): Promise<IDoctorDocument>;
    updateDoctorDocuments(userId: string, files: { [fieldname: string]: Express.Multer.File[] }): Promise<IDoctorDocument>;
