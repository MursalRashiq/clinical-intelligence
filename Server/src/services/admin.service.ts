...
import { LoginAdminDTO, UserFilterDTO, DoctorRequestDTO, DoctorRequestDetailsDTO, DoctorFilterDTO } from "../dtos/admin.dto/admin.dto";
...
export class AdminService implements IAdminService {
  ...
  async getAllDoctors(filters: DoctorFilterDTO): Promise<{ doctors: DoctorListItem[]; total: number; page: number; limit: number; totalPages: number; }> {
    const page = filters.page || 1;
    const limit = filters.limit || 10;
    const { skip } = calculatePagination(page, limit);
  ...
  async getDoctorRequestDetails(doctorId: string, baseUrl?: string): Promise<DoctorRequestDetailsDTO | null> {
    const doc = await this._doctorRepository.getDoctorRequestDetailsById(doctorId);
    if (!doc || !doc.userId) {
      return null
    }
  ...
  async rejectDoctorRequest(doctorId: string, rejectionReason: string): Promise<void> {
    await this._doctorRepository.updateById(doctorId, {
