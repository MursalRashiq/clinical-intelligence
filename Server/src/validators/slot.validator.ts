...
import { DayOfWeek } from '../types/slot.type';
...
export class SlotValidator {
  ...
  static validateCreateSchedule(data: CreateScheduleDTO): void {
    ...
    if (!data.weeklySchedule || !Array.isArray(data.weeklySchedule)) {
      throw new ValidationError(
        'Weekly schedule is required and must be an array',
      );
    }
  ...
  static validateBlockDate(data: BlockDatesDTO): void {
    if (!data.date) {
  ...
  static validateSpecificDateSlots(data: {
    date: Date | string;
    startTime: string;
    endTime: string;
  }): void {
    ...
    if (targetDate < today) {
      throw new ValidationError('Cannot create slots in the past');
    }

    this.validateTimeSlot({ startTime: data.startTime, endTime: data.endTime });
  }

  private static validateTimeSlot(slot: {
    startTime: string;
    endTime: string;
  }): void {
  ...
  private static validateNoOverlappingSlots(
    ...
    for (let i = 0; i < slots.length; i++) {
      for (let j = i + 1; j < slots.length; j++) {
        ...
        // Existence check to satisfy noUncheckedIndexedAccess: true
  ...
  private static timeToMinutes(time: string): number {
    ...
    // Provide defaults or check for existence because of noUncheckedIndexedAccess
    const h = hours ?? 0;
    const m = minutes ?? 0;
