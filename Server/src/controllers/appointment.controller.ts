...
import { IAppointmentController } from './interface/IAppointment.controller';
import { IAppointmentService } from '../services/interface/IAppointment.service';
...
const getSingleString = (val: string | string[] | undefined): string => {
  if (!val) return '';
...
export class AppointmentController implements IAppointmentController {
  ...
  createAppointment = async (
    ...
    try {
      const patientId = getSingleString(req.user?.userId);
      if (!patientId) {
      ...
      this.logger.info('Calling createAppointment', {
        patientId,
        appointmentData,
      });
  ...
  getMyAppointments = async (
    ...
    try {
      ...
      const userRole = getSingleString(req.user?.role);
  ...
  getAppointmentById = async (
    ...
    try {
      const userId = getSingleString(req.user?.userId);
      const userRole = getSingleString(req.user?.role);
      const appointmentId = getSingleString(req.params.id);
  ...
  rejectReschedule = async (
    ...
    try {
      ...
      await this._appointmentService.rejectReschedule(
        appointmentId,
        userId,
        reason,
      );
