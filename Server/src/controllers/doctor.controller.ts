...
export class DoctorController implements IDoctorController {
  ...
  getDoctorProfile = async (
    ...
    try {
      ...
      const result = await this._doctorService.getDoctorProfile(
        userId.toString(),
      );
      sendSuccess(
        res,
  ...
  getAllDoctors = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log('Fetching doctors with filters:', filter);
  ...
  resubmitVerification = async (
    ...
    try {
      ...
      const result = await this._doctorService.resubmitVerification(
        userId.toString(),
      );
      sendSuccess(
        res,
        result,
        'Doctor verification resubmitted successfully',
        HttpStatus.OK,
      );
    } catch (err: unknown) {
  ...
  updateDocuments = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const files = req.files as { [fieldname: string]: Express.Multer.File[] };
      const result = await this._doctorService.updateDoctorDocuments(
        userId.toString(),
        files,
      );
      sendSuccess(
        res,
        result,
        'Doctor documents updated successfully',
        HttpStatus.OK,
      );
    } catch (err: unknown) {
  ...
  getDocumentUrl = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!indexStr || typeof indexStr !== 'string') {
        return next(new Error('Invalid document index'));
      }
      const index = parseInt(indexStr, 10);
      const result = await this._doctorService.getDocumentSignedUrl(
        userId.toString(),
        index,
      );
      sendSuccess(
