...
export const appointmentService = {
    ...
    createAppointment: async (appointmentData: AppointmentData) => {
        ...
        // Server wraps in { success, data: appointment }
        return response.data?.data ? { success: true, data: response.data.data } : response.data;
    },
    ...
    getMyAppointments: async (status?: string, page: number = 1, limit: number = 10, search?: string) => {
        ...
        // Server wraps in { success, data: { appointments, total, counts } }
